...
Add `inertiaValidator(target, schema, options?)` — a zero-config [Standard Schema](https://standardschema.dev/) validator that wraps `@hono/standard-validator`'s `sValidator` and automatically calls `back(c, { errors: toInertiaErrors(issues) })` on failure. Exported from the `inertia-hono/validator` subpath so the main bundle stays free of validator code (and `@hono/standard-validator` is an optional peer dep). Supports `{ errors, back }` options that forward to `toInertiaErrors` and `back()` respectively.
